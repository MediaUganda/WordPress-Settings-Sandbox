<?php
/**
 * Plugin Name: A settings options page in OOP.
 * Plugin URI: https://mediauganda.com
 * Author: Media Uganda
 * Author URI: https://mediauganda.com
 * Description: TA settings options page in OOP.
 * Version: 0.1.0
 * License: 0.1.0
 ...
 * 
 * Credits
 * Forked: WordPress Settings Sandbox - http://github.com/tommcfarlin/wordpress-settings-sandbox
...
if ( ! class_exists( 'SettingsPage' ) ) {
	...
	class SettingsPage {
		...
		public function register() {
			add_action( 'admin_menu', [ $this, 'sandbox_example_theme_menu' ] );
			add_action( 'admin_init', [ $this, 'sandbox_theme_initialize_input_examples' ], 11 );
			add_action( 'admin_init', [ $this, 'sandbox_theme_initialize_social_options' ], 11);
			add_action( 'admin_init', [ $this, 'sandbox_initialize_theme_options' ], 10 );
		}
		...
		function sandbox_example_theme_menu() {
			...
			add_theme_page(
				...
				[ $this, 'sandbox_theme_display' ]				// The name of the function to call when rendering this menu's page
			);
			...
			add_menu_page(
				...
				[ $this, 'sandbox_theme_display' ]				// The callback function used to render this menu
			);
			...
			add_submenu_page(
				...
				[ $this, 'sandbox_theme_display' ]				// The callback function used to render the options for this submenu item
			);
		...
		* ------------------------------------------------------------------------ */ 
		...
		function sandbox_initialize_theme_options() {
			...
			if( false == get_option( 'sandbox_theme_display_options' ) ) {	
				add_option( 'sandbox_theme_display_options', apply_filters( 'sandbox_theme_default_display_options', [ $this, 'sandbox_theme_default_display_options' ] ) );
			} // end if
			...
			add_settings_section(
				...
				[ $this, 'sandbox_general_options_callback' ],	// Callback used to render the description of the section
			...
			add_settings_field(	
				...
				[ $this, 'sandbox_toggle_header_callback' ],	// The name of the function responsible for rendering the option interface
			...
			add_settings_field(	
				'show_content',						
				__( 'Content', 'sandbox' ),				
				[ $this, 'sandbox_toggle_content_callback' ],	
			...
			add_settings_field(	
				'show_footer',						
				__( 'Footer', 'sandbox' ),				
				[ $this, 'sandbox_toggle_footer_callback' ],	
				'sandbox_theme_display_options',		
				'general_settings_section',			
				array(								
		...
		function sandbox_theme_initialize_social_options() {
			...
			if( false == get_option( 'sandbox_theme_social_options' ) ) {	
				add_option( 'sandbox_theme_social_options', apply_filters( 'sandbox_theme_default_social_options', [$this, 'sandbox_theme_default_social_options' ] ) );
			} // end if
			
			add_settings_section(
				'social_settings_section',			// ID used to identify this section and with which to register options
				__( 'Social Options', 'sandbox' ),		// Title to be displayed on the administration page
				[ $this, 'sandbox_social_options_callback' ],	// Callback used to render the description of the section
			...
			add_settings_field(	
				'facebook',						
				'Facebook',							
				[ $this, 'sandbox_facebook_callback' ],	
				'sandbox_theme_social_options',	
				'social_settings_section'			
			);
			...
			add_settings_field(	
				'googleplus',						
				'Google+',							
				[ $this, 'sandbox_googleplus_callback' ],	
				'sandbox_theme_social_options',	
				'social_settings_section'			
			);
		...
		function sandbox_theme_initialize_input_examples() {
			...
			if( false == get_option( 'sandbox_theme_input_examples' ) ) {	
				add_option( 'sandbox_theme_input_examples', apply_filters( 'sandbox_theme_default_input_options', [ $this, 'sandbox_theme_default_input_options' ] ) );
			} // end if

			add_settings_section(
				'input_examples_section',
				__( 'Input Examples', 'sandbox' ),
				[ $this, 'sandbox_input_examples_callback' ],
				'sandbox_theme_input_examples'
			);
			...
			add_settings_field(	
				...
				[ $this, 'sandbox_input_element_callback' ],	
				'sandbox_theme_input_examples',	
				'input_examples_section'			
			);
			...
			add_settings_field(	
				...
				[ $this, 'sandbox_textarea_element_callback' ],	
				'sandbox_theme_input_examples',	
				'input_examples_section'			
			);
			...
			add_settings_field(
				...
				[ $this, 'sandbox_radio_element_callback' ],
				'sandbox_theme_input_examples',
				'input_examples_section'
			);
			...
			add_settings_field(
				...
				[ $this, 'sandbox_select_element_callback' ],
				'sandbox_theme_input_examples',
				'input_examples_section'
			);
		...
		* ------------------------------------------------------------------------ */ 
		...
		function sandbox_input_element_callback() {
			...
			$input_example = '';
			if( isset( $options['input_example'] ) ) {
				$input_example = esc_url( $options['input_example'] );
			} // end if
			
			// Render the output
			echo '<input type="text" id="input_example" name="sandbox_theme_input_examples[input_example]" value="' . $input_example . '" />';
		...
		function sandbox_textarea_element_callback() {
			...
			$textarea_example = '';
			if( isset( $options['textarea_example'] ) ) {
				$textarea_example = esc_url( $options['textarea_example'] );
			} // end if
			
			// Render the output
			echo '<textarea id="textarea_example" name="sandbox_theme_input_examples[textarea_example]" rows="5" cols="50">' . $textarea_example . '</textarea>';
		...
		function sandbox_checkbox_element_callback() {
			...
			$checkbox_example = 0;
			if( isset( $options['checkbox_example'] ) ) {
				$checkbox_example = esc_url( $options['checkbox_example'] );
			} // end if

			$html = '<input type="checkbox" id="checkbox_example" name="sandbox_theme_input_examples[checkbox_example]" value="1"' . checked( 1, $checkbox_example, false ) . '/>';
			$html .= '&nbsp;';
		...
		function sandbox_radio_element_callback() {
			...
			$radio_example = 0;
			if( isset( $options['radio_example'] ) ) {
				$radio_example = esc_url( $options['radio_example'] );
			} // end if
			
			$html = '<input type="radio" id="radio_example_one" name="sandbox_theme_input_examples[radio_example]" value="1"' . checked( 1, $radio_example, false ) . '/>';
			$html .= '&nbsp;';
			$html .= '<label for="radio_example_one">Option One</label>';
			$html .= '&nbsp;';
			$html .= '<input type="radio" id="radio_example_two" name="sandbox_theme_input_examples[radio_example]" value="2"' . checked( 2, $radio_example, false ) . '/>';
			$html .= '&nbsp;';
		...
		function sandbox_select_element_callback() {
			...
			$time_options = 'default';
			if( isset( $options['time_options'] ) ) {
				$time_options = esc_url( $options['time_options'] );
			} // end if
			...
			$html = '<select id="time_options" name="sandbox_theme_input_examples[time_options]">';
				...
				$html .= '<option value="never"' . selected( $time_options, 'never', false) . '>' . __( 'Never', 'sandbox' ) . '</option>';
				$html .= '<option value="sometimes"' . selected( $time_options, 'sometimes', false) . '>' . __( 'Sometimes', 'sandbox' ) . '</option>';
				$html .= '<option value="always"' . selected( $time_options, 'always', false) . '>' . __( 'Always', 'sandbox' ) . '</option>';	$html .= '</select>';
